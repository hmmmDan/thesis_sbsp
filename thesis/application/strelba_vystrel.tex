...
Metoda \texttt{TryShoot()} se volá z \texttt{PlayerController} při inputu střelby. Kontroluje se několik podmínek: zda je střelba odemčená (volání \texttt{IsUnlocked()}), zda neprobíhá přebíjení (proměnná \texttt{isReloading}) a zda jsou k dispozici náboje (\texttt{currentShots > 0}).
