...
Když projektil zasáhne kouli, volá se metoda \texttt{Stun()}. Ta inkrementuje counter \texttt{\_activeStuns} a spustí korutinu \texttt{StunCooldown()}. Systém rozlišuje první stun a další stuny -- u prvního se uloží aktuální rychlost jako základ, u dalších se jen nastaví nízká rychlost.
...
Tento systém umožňuje hráči střílet více projektilů za sebou pro delší zpomalení, ale vzhledem k omezené munici (2 náboje) to je neefektivní. Navíc po každém stunu se základní rychlost zvýší, takže koule je postupně rychlejší.
